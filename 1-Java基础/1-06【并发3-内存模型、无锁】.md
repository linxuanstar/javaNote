...
> synchronized不能够在非final类型的成员变量上面使用（语法上面没有问题，但是一定要在final类型的成员变量上面使用。这样避免重新赋值，导致不同的线程使用不同的对象作为锁，达不到同步锁定的效果。），更不能锁住值为NULL的变量。我们知道synchronized本质就是一个monitor，在内存中找一个对象关联，这样值是NULL，那么根本找不到。
