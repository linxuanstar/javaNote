...
1. 每次指向到synchronized代码块时，都会创建锁记录（Lock Record）对象，每个线程都会包括一个锁记录的结构，锁记录内部可以储存对象的Mark Word和对象引用reference
   ...
   ![1583755737580](https://piplong-img.oss-cn-hangzhou.aliyuncs.com/blog-img/202206261904048.png)

2. 让锁记录中的Object reference指向对象，并且尝试用cas(compare and sweep)替换Object对象的Mark Word ，将Mark Word 的值存入锁记录中

   ![1583755888236](https://piplong-img.oss-cn-hangzhou.aliyuncs.com/blog-img/202206261904627.png)

3. 如果cas替换成功，那么对象的对象头储存的就是锁记录的地址和状态00，如下所示

   ![1583755964276](https://piplong-img.oss-cn-hangzhou.aliyuncs.com/blog-img/202206261904394.png)
...
4. 如果cas失败，有两种情况
   ...
   2. 如果是自己的线程已经执行了synchronized进行加锁，那么那么再添加一条 Lock Record 作为重入的计数
      ...
      ![1583756190177](https://piplong-img.oss-cn-hangzhou.aliyuncs.com/blog-img/202206261904108.png)
...
5. 当线程退出synchronized代码块的时候，
   ...
   ![1583756357835](https://piplong-img.oss-cn-hangzhou.aliyuncs.com/blog-img/202206261904255.png)
