...
`Stack`只有入栈和出栈的操作。在Java中，我们用`Deque`可以实现`Stack`的功能。为什么Java的集合类没有单独的`Stack`接口呢？因为有个遗留类名字就叫`Stack`，出于兼容性考虑，所以没办法创建`Stack`接口，只能用`Deque`接口来“模拟”一个`Stack`了。

| 方法                  | 作用               |
| --------------------- | ------------------ |
| push(E) / addFirst(E) | 把元素压栈         |
| pop() / removeFirst() | 取栈顶元素并且弹出 |
| peek() / peekFirst()  | 取栈顶元素但不弹出 |
